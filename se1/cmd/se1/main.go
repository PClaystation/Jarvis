...
func firstRunEnroll(cfgPath string, serverBaseURL string, deviceIDInput string, displayNameInput string, bootstrapToken string, version string) (*config.Config, error) {
	if serverBaseURL == "" {
		return nil, errors.New("missing server URL; pass --server-url or set SE1_SERVER_URL")
	}

	if bootstrapToken == "" {
		return nil, errors.New("missing bootstrap token; pass --bootstrap-token or set SE1_BOOTSTRAP_TOKEN")
	}
	...
	if autoDesignate {
		deviceID = ""
	} else if !strings.HasPrefix(deviceID, "se") {
		deviceID = config.SanitizeDeviceID("se-" + deviceID)
	}
