...
func stagePackage(request updateRequest) (string, error) {
	dir := os.TempDir()
	stagePath := filepath.Join(dir, fmt.Sprintf("e1-agent-update-%d.exe", time.Now().UTC().UnixNano()))
...
func writeUpdaterScript(targetPath string, stagedPath string, cfgPath string, launchConfigPath string, version string) (string, error) {
	scriptPath := filepath.Join(os.TempDir(), fmt.Sprintf("e1-agent-updater-%d.cmd", time.Now().UTC().UnixNano()))
