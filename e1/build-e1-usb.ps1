param(
  ...
  [string]$OutputPath = ".\dist\e1-agent-usb.exe",
...
try {
  $resourceState = New-CordycepsWindowsBuildResource `
    -RepoRoot $repoRoot `
    -PackageDir (Join-Path $scriptRoot "cmd/e1") `
    -Version $Version `
    -ProductName "Cordyceps E1 Agent" `
    -FileDescription "Cordyceps E1 USB-ready Windows agent" `
    -OriginalFilename (Split-Path -Leaf $outputFullPath) `
    -InternalName "e1-agent"
