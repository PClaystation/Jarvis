...
func firstRunEnroll(cfgPath string, serverBaseURL string, deviceIDInput string, bootstrapToken string, version string) (*config.Config, error) {
	...
	if bootstrapToken == "" {
		return nil, errors.New("missing bootstrap token; pass --bootstrap-token or set A1_BOOTSTRAP_TOKEN")
	}
	...
	if autoDesignate {
		deviceID = ""
	} else if !strings.HasPrefix(deviceID, "a") {
		deviceID = config.SanitizeDeviceID("a-" + deviceID)
	}
