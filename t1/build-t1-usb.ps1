param(
  ...
  [string]$OutputPath = ".\dist\t1-agent-usb.exe",
...
try {
  $resourceState = New-CordycepsWindowsBuildResource `
    -RepoRoot $repoRoot `
    -PackageDir (Join-Path $scriptRoot "cmd/t1") `
    -Version $Version `
    -ProductName "Cordyceps T1 Agent" `
    -FileDescription "Cordyceps T1 USB-ready Windows agent" `
    -OriginalFilename (Split-Path -Leaf $outputFullPath) `
    -InternalName "t1-agent"
