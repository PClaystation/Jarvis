param(
  ...
  [string]$OutputPath = ".\dist\s1-agent-usb.exe",
...
try {
  $resourceState = New-CordycepsWindowsBuildResource `
    -RepoRoot $repoRoot `
    -PackageDir (Join-Path $scriptRoot "cmd/s1") `
    -Version $Version `
    -ProductName "Cordyceps S1 Agent" `
    -FileDescription "Cordyceps S1 USB-ready Windows agent" `
    -OriginalFilename (Split-Path -Leaf $outputFullPath) `
    -InternalName "s1-agent"
